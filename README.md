...
```sh
make docker-build docker-push IMG=<some-registry>/custom-scheduler:tag
```
...
```sh
make deploy IMG=<some-registry>/custom-scheduler:tag
```
...
```sh
make build-installer IMG=<some-registry>/custom-scheduler:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/custom-scheduler/<tag or branch>/dist/install.yaml
```
